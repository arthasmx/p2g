var business = {
  msg:{
    saving       : "Guardando empresa, espere por favor...",
    error_saving : "No fue posible guardar el empresa. Favor de reportar este error lo mas pronto posible.",
  ...
  form:{
    ...
    form        : "form#add-business",
    ckeditor_business : "article"
  },
  url:{
    listing             : baseUrl + '/business/list',
    save                : baseUrl + '/business/save',
    listing_status      : baseUrl + '/business/listing-status',
    listing_lang        : baseUrl + '/business/listing-language',
    listing_promote     : baseUrl + '/business/listing-promote',
    listing_mobile      : baseUrl + '/business/listing-mobile',
    links_rel           : baseUrl + '/business/links-rel',
    save_coordinates    : baseUrl + '/business/save-coordinates',
    del_coordinates     : baseUrl + '/business/del-coordinates',
    del_image           : baseUrl + '/business-fi/business-delete-image',
    gallery_reload      : baseUrl + '/business-fi/business-reload-gallery'
  },
  ...
  save:function(button){
    ...
    var params = jQuery(self.form.form).serialize() + "&action=" + action + "&btn="+ button +"&article=" + escape(CKEDITOR.instances[business.form.ckeditor_business].getData());
    jQuery.ajax({
      type: 'POST',
      data: params,
      dataType: 'json',
  ...
  process_save_response:function(response,button){
    ...
    if( is_number(response) ){
      ...
      case 'next':
        blockUI_ajax_saving(self.tabs.id,"off");
        jQuery( business.button.save +','+ business.button.save_new +','+ business.button.save_close +','+ business.button.publicate ).removeClass('hide');
        jQuery( business.button.next ).addClass('hide');
      ...
      default:
        ...
        jQuery.each(response, function(i,item) {
          ...
          if( response[i].duplicated ){
            if( response[i].duplicated =='username' ){
              jQuery( self.form.form + " dl.field-username" + " dt label").addClass("missing-field").html('Usuario ya existe!');
            }
            if( response[i].duplicated =='email' ){
              jQuery( self.form.form + " dl.field-email" + " dt label").addClass("missing-field").html('Correo ya existe!');
            }
          }

          if( response.unmatch ){
            jQuery( self.form.form + " dl.field-pass dt label," + self.form.form + " dl.field-confirmation dt label").addClass("missing-field").html('Clave no coincide');
          }
  ...
  clear_elements:function(){
    ...
    ckeditor_clear(self.form.ckeditor_business);
    ...
    jQuery(business.dom.map_coors).attr("value","").removeAttr("data-initialized");
...
//    jQuery(self.form.date_public).datepicker('setDate', new Date());
    jQuery( business.button.next ).removeClass('hide');
    jQuery( business.button.save +','+ business.button.save_new +','+ business.button.save_close +','+ business.button.publicate ).addClass('hide');
  },
...
jQuery(document).ready(function(){
  /* tabs click */
  jQuery( business.tabs.id + " ul li").click(function(){
    if( ! jQuery( business.form.article_id ).is("[data-id]") ){
      jQuery(business.tabs.id).tabs('select', 0);
    }
    if( jQuery(this).index() == 2 && ! jQuery(business.dom.map_coors).is('[data-initialized]') ){
      initialize();
    }
  });

  /* save */
  jQuery(business.button.next+','+business.button.save +','+ business.button.save_new +','+ business.button.save_close +','+ business.button.publicate).click(function(){
    business.save( jQuery(this).attr('id') );
  });
  /* coordinates add */
  jQuery(business.button.coordinates).click(function() {
    if ( ! jQuery(business.dom.current_cors).is(':empty') ){
      business.save_coordinates( jQuery(business.dom.current_cors).text() );
    }
  });
  /* coordinates del */
  jQuery(business.button.del_coordinates).click(function() {
    business.del_coordinates();
  });
  /* links rel */
  jQuery(business.button.links_rel).click(function() {
    business.save_links_rel();
  });
  /* delete uploaded images */
  jQuery(document).on('click', business.dom.del_image_span, function(){
    business.del( jQuery(this).attr('data-id'), jQuery(this).parent().parent().index() );
  });
