...
class Module_Email_Repository_Model_Business_Register extends Module_Email_Repository_Model_Email {

  protected $firma      =  "Atención al usuario";  // ¿Quién firma el email?, se usa en la plantilla
  protected $remitente  =  "noreply";              // Remitentes definidos en /Module/Email/etc/core.ini
  protected $asunto     =  "Mensaje recibido desde página Contacto";
  protected $to         =  false;
  ...
  // Accede al sender del email (Zend_Mail), lo configura, carga el contenido de las plantillas y envía el email.
    ...
    // Generamos el contenido del email (todos los atributos en $this se asignan a la vista del bloque de la plantilla)
      $this->body=array(
        "html"  =>  $this->_module->renderBlock('business/register'  , 'html'  , $this->asArray() ),
        "txt"   =>  $this->_module->renderBlock('business/register'  , 'txt'   , $this->asArray() ),
      );
