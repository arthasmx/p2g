<?php
class Module_Acl_Repository_Resource_Acl_Activate extends Core_Model_Repository_Model {
...
/* Actualizaciones ************************/
	...
	function douse($access_ip) {
		...
		$query=sprintf("UPDATE acl_account_activate set access_date=NOW(), access_ip='%s', used='1' where hash='%s'", $access_ip, $this->hash);
	...
	function activate($user=false) {
		...
		$query=sprintf("UPDATE acl set status=1, updated=NOW() where username='%s'", $user);
		$this->_db->query($query);
		return true;
	}
...
/* Inserciones ************************/
	...
	/**
	 * Guardamos el HASH para la activacion de un registro
	 *
	 ...
	 * @return unknown
	 */
	function createAccountActivation($hash,$username,$request_ip,$expires) {
		$query=sprintf("INSERT INTO acl_account_activate set hash='$hash',username='$username',request_ip='$request_ip',request_date=NOW(),expires='$expires'");
		$this->_db->query($query);
