...
class Module_Acl_Repository_Model_Acl extends Core_Model_Repository_Model {
    ...
    function init() {
        ...
        if (!$this->_resource) {
            $this->_resource=$this->_module->getResourceSingleton('acl');
        }
...
// MAIN ****************************************************************************************
    ...
    function login($user,$pwd) {
        ...
        if ( ! $this->isLogged()) {
            ...
            if ( $this->_resource->reset()->setUserName($user)->setPasswd(md5($pwd))->getRow() ) {
                $this->user = $user;
                $this->life = time()+self::LIFE;
    ...
    /**
     * Decidimos a que area enviar al usuario dependiendo de sus privilegios
     *
     * @return none
     */
    function loginRedirectArea(){
        if(!$this->isLogged()) return false;

        // Contamos los privilegios del cliente. Si tiene mas de 1, llevarlo a una pagina donde
        // Seleccione el area a la cual desea entrar
        // Si tiene solo 1, entonces llevarlo a la que le corresponde
        if(sizeof($this->data['privileges'])>1){
            ...
            switch ( $this->data['privileges'][0]['privilege'] ) {
                ...
                case 2:
                        // Empresas
                        header('Location:' . App::base('/business') );
                        exit;
                        break;
                case 3:
                        // Admin
