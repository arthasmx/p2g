...
// INICIALIZACION ******************************************************************************************
	...
	public function etiquetas(){
		...
		define('FPDF_FONTPATH',FP.DS."font".DS);
			...
			$col=1; $ren=1; $renglon=1;
			foreach($this->clientes as $cliente){

					//$pdf->SetXY(0,1);
					$pdf->SetXY($col,$ren);
					$pdf->MultiCell(0, .3, utf8_decode($cliente['nombre'].PHP_EOL.$cliente['direccion'].PHP_EOL.$cliente['colonia'].PHP_EOL.$cliente['ciudad'].', '.$cliente['estado'].PHP_EOL.$cliente['cp'].PHP_EOL.$cliente['pais']),250,'J' );
		...
		//Create file { F= guarda el archivo. I=Lo lanza al navegador}
			//$pdf->Output( WP.DS."etiquetas".DS."etiquetas.pdf" ,"F");
			$name = 'Etiqueta del '.$this->_module->getResource('Dates')->toDate(7, date('Y-m-d h:i:s'));
			$pdf->Output( WP.DS."etiquetas".DS.$name.".pdf" ,"F");
